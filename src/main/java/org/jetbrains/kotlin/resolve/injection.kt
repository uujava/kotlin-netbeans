...
private fun createContainerForLazyResolveWithJava(
    ...
    configureModule(moduleContext, JvmPlatform, JvmTarget.JVM_1_8, bindingTrace)
    configureJavaTopDownAnalysis(moduleContentScope, moduleContext.project, lookupTracker, languageVersionSettings)
    useInstance(KotlinEnvironment.getEnvironment(project).createPackagePartProvider(moduleContentScope))
    ...
    if (useLazyResolve) {
        throw UnsupportedOperationException("useLazyResolve")
    }
}.apply {
...
fun createContainerForTopDownAnalyzerForJvm(
        ...
        CompilerEnvironment, lookupTracker, languageVersionSettings,
